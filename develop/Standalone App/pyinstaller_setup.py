...
Porpose: Provide build options to bundle the FFaudiocue application.
...
try:
    from ffaudiocue.ffc_sys import about_app
except ModuleNotFoundError as modulerror:
    sys.exit(f'ERROR: {modulerror}\nMake sure to cd to FFaudiocue source '
             f'directory first.')
SCRIPT = 'launcher'
...
def build_language_catalog():
    ...
    if quest in ('y', 'Y'):
        try:
            cmd = compile_catalog()
            cmd.directory = os.path.join('ffaudiocue', 'data', 'locale')
            cmd.domain = "ffaudiocue"
            cmd.statistics = True
...
def ffaudiocue_data_source(name=NAME, release=about_app):
    """
    Returns a dict object of the FFaudiocue data
    ...
    return {"RLS_NAME": release.RELNAME,  # first letter is Uppercase
            ...
            "EMAIL": release.MAIL,
            "ART": os.path.join('ffaudiocue', 'data', 'icons'),
            "LOCALE": os.path.join('ffaudiocue', 'data', 'locale'),
            "FFMPEG": os.path.join('ffaudiocue', 'data', 'FFMPEG'),
            "NOTICE": os.path.join('ffaudiocue', 'data',
                                   'FFMPEG', 'NOTICE.rtf'),
            "AUTH": 'AUTHORS',
            "BUGS": 'BUGS',
            "CHANGELOG": 'CHANGELOG',
            ...
            "README": 'README.md',
            "TODO": 'TODO',
            "ICNS": os.path.join('ffaudiocue',
                                 'data',
                                 'icons',
                                 'ffaudiocue.icns'),
            "ICO": os.path.join('ffaudiocue',
                                'data',
                                'icons',
                                'ffaudiocue.ico')
            }
...
class PyinstallerSpec():
    ...
    and manages the options to generate a ffaudiocue.spec
    ...
    def __init__(self, onedf='--onedir'):
        ...
        self.getdata = ffaudiocue_data_source()
    ...
    def darwin_platform(self):
        ...
        plist = (
            f"""             info_plist={{# 'LSEnvironment': '$0',
             ...
             'CFBundleIdentifier': "com.jeanslack.ffaudiocue",
...
def fetch_exec(binary=BINARY):
    """
    fetch the ffaudiocue binary on bin folder
    """
...
def genspec(options, specfile=SPECFILE, addplist=None, script=SCRIPT):
    """
    Generate a ffaudiocue.spec file for platform in use.
    Support for the following platforms is expected:
            [Windows, Darwin, Linux]
    The ffaudiocue.spec file will be saved in the root directory
    of the ffaudiocue sources.
    To running ffaudiocue.spec is required ``pyinstaller``.
    To use ffaudiocue.spec type:
        `pyinstaller ffaudiocue.spec`
    or use this script with option -s to start the building by
    an existing ffaudiocue.spec file.
    """
    try:
...
def run_pyinst(specfile=SPECFILE, this=THIS):
    """
    wrap `pyinstaller --clean ffaudiocue.spec`
    """
    if os.path.exists(specfile) and os.path.isfile(specfile):
        fetch_exec()  # fetch ffaudiocue binary
        time.sleep(1)
        try:
...
def make_portable():
    ...
    Note:
       ...
       be inside the ffaudiocue standalone directory.
        with `--onefile` option, the 'portable_data' directory should
        be next to the ffaudiocue standalone executable.
    """
    while True:
    ...
    filename = os.path.join('ffaudiocue', 'gui_app.py')
...
def get_data_platform():
    ...
    if not platform.system() in ('Windows', 'Darwin', 'Linux'):
        sys.exit("\nERROR: Unsupported platform.\n"
                 ...
                 "following command:\n"
                 "\"pyi-makespec [options] ffaudiocue.py\"\n"
                 )
...
def main():
    ...
    """
    descr = ('Provides build options to help bundle a FFaudiocue application. '
    ...
    parser.add_argument(
        '-g', '--gen-spec',
        help=("Generate a `ffaudiocue.spec` file only. A *.spec file is "
    ...
    parser.add_argument(
        ...
        help=("Build the bundle application. Can be used with `--gen-spec` "
              ...
              "`ffaudiocue.spec` file if not present or if you want to "
