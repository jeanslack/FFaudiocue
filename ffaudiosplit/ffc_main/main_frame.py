...
from ffaudiosplit.ffc_utils.get_bmpfromsvg import get_bmp
from ffaudiosplit.ffc_dlg import preferences
from ffaudiosplit.ffc_dlg import infoprg
from ffaudiosplit.ffc_dlg.cd_info import CdInfo
from ffaudiosplit.ffc_dlg.track_info import TrackInfo
from ffaudiosplit.ffc_dlg import check_new_version
from ffaudiosplit.ffc_dlg.showlogs import ShowLogs
from ffaudiosplit.ffc_panels import cuesplitter_panel
from ffaudiosplit.ffc_inout import io_tools
from ffaudiosplit.ffc_sys.about_app import VERSION
...
class MainFrame(wx.Frame):
    ...
    def __init__(self):
        ...
        # ----------------------Set Properties----------------------#
        self.SetTitle("FFaudiosplit")
        icon = wx.Icon()
        icon.CopyFromBitmap(wx.Bitmap(self.icons['ffaudiosplit'],
                                      wx.BITMAP_TYPE_ANY))
        self.SetIcon(icon)
    ...
    def frame_menu_bar(self):
        ...
        if self.appdata['showhidenmenu'] is True:
            ...
            dscrp = (_("Configuration Directory"),
                     _("Opens the FFaudiosplit configuration directory"))
        ...
        dscrp = (_("Check for newer version"),
                 _("Checks for the latest FFaudiosplit version at "
                   "<https://github.com/jeanslack/FFaudiosplit>"))
        checkitem = help_button.Append(wx.ID_ANY, dscrp[0], dscrp[1])
        help_button.AppendSeparator()
        infoitem = help_button.Append(wx.ID_ABOUT,
                                      _("About FFaudiosplit"), "")
        ...
        if os.path.exists(fname) and os.path.isfile(fname):
            ...
            try:
                ...
                wx.MessageBox(_("Unexpected error while creating file:\n\n"
                                "{0}").format(err),
                              'FFaudiosplit', wx.ICON_ERROR, self)
            else:
                io_tools.openpath(fname)
    # ------------------------------------------------------------------#
    # --------- Menu  Go  ###
    ...
    def open_log_dir(self, event):
        ...
        if not os.path.exists(self.appdata['logdir']):
            wx.MessageBox(_("There are no logs to show."),
                          "FFaudiosplit", wx.ICON_INFORMATION, self)
            return
    ...
    def help_me(self, event):
        ...
        """
        page = 'https://github.com/jeanslack/ffaudiosplit'
    ...
    def wiki(self, event):
        ...
        page = 'https://github.com/jeanslack/FFaudiosplit/wiki'
        webbrowser.open(page)
    # ------------------------------------------------------------------#

    def issues(self, event):
        """Display issues page on github"""
        page = 'https://github.com/jeanslack/ffaudiosplit/issues'
        webbrowser.open(page)
    # ------------------------------------------------------------------#
    ...
    def check_new_releases(self, event):
        """
        Compare the FFaudiosplit version with a given
        new version found on github.
        """
        this = VERSION  # this version
        url = ("https://api.github.com/repos/jeanslack/"
               "FFaudiosplit/releases/latest")
    ...
    def show_infoprog(self, event):
        ...
        """
        infoprg.info_gui(self, self.icons['ffaudiosplit'])
