...
Porpose: Set FFaudiosplit configuration on startup
...
def create_dirs(dirname, fconf):
    ...
    of directories required for FFaudiosplit if they do
    not exist.
    Returns dict:
        key == 'R'
        key == ERROR (if any errors)
...
def conventional_paths():
    ...
    if platform.system() == 'Windows':
        fpath = "\\AppData\\Roaming\\ffaudiosplit\\settings.json"
        file_conf = os.path.join(user + fpath)
        dir_conf = os.path.join(user + "\\AppData\\Roaming\\ffaudiosplit")
        log_dir = os.path.join(dir_conf, 'log')  # logs

    elif platform.system() == "Darwin":
        fpath = "Library/Application Support/ffaudiosplit/settings.json"
        file_conf = os.path.join(user, fpath)
        dir_conf = os.path.join(user, os.path.dirname(fpath))
        log_dir = os.path.join(user, "Library/Logs/ffaudiosplit")

    else:  # Linux, FreeBsd, etc.
        fpath = ".config/ffaudiosplit/settings.json"
        file_conf = os.path.join(user, fpath)
        dir_conf = os.path.join(user, ".config/ffaudiosplit")
        log_dir = os.path.join(user, ".local/share/ffaudiosplit/log")
...
class DataSource():
    """
    DataSource class determines the FFaudiosplit's
    ...
    def __init__(self, kwargs):
        ...
        if getattr(sys, 'frozen', False) and hasattr(sys, '_MEIPASS'):
            ...
            srcdata = os.path.join(sitepkg, 'ffaudiosplit', 'data')
            self.dataloc["app"] = None
            msg(f"Info: Package: «ffaudiosplit»\n"
    ...
    def icons_set(self, icontheme):
        ...
        """
        keys = ('ffaudiosplit', 'startsplit', 'setup',
