...
Porpose: Set FFaudiocue configuration on startup
...
def create_dirs(dirname, fconf):
    ...
    of directories required for FFaudiocue if they do
    not exist.
    Returns dict:
        key == 'R'
        key == ERROR (if any errors)
...
def conventional_paths():
    ...
    if platform.system() == 'Windows':
        fpath = "\\AppData\\Roaming\\ffaudiocue\\settings.json"
        file_conf = os.path.join(user + fpath)
        dir_conf = os.path.join(user + "\\AppData\\Roaming\\ffaudiocue")
        log_dir = os.path.join(dir_conf, 'log')  # logs

    elif platform.system() == "Darwin":
        fpath = "Library/Application Support/ffaudiocue/settings.json"
        file_conf = os.path.join(user, fpath)
        dir_conf = os.path.join(user, os.path.dirname(fpath))
        log_dir = os.path.join(user, "Library/Logs/ffaudiocue")

    else:  # Linux, FreeBsd, etc.
        fpath = ".config/ffaudiocue/settings.json"
        file_conf = os.path.join(user, fpath)
        dir_conf = os.path.join(user, ".config/ffaudiocue")
        log_dir = os.path.join(user, ".local/share/ffaudiocue/log")
...
class DataSource():
    """
    DataSource class determines the FFaudiocue's
    ...
    def __init__(self, kwargs):
        ...
        if getattr(sys, 'frozen', False) and hasattr(sys, '_MEIPASS'):
            ...
            srcdata = os.path.join(sitepkg, 'ffaudiocue', 'data')
            self.dataloc["app"] = None
            msg(f"Info: Package: «ffaudiocue»\n"
    ...
    def icons_set(self, icontheme):
        ...
        """
        keys = ('ffaudiocue', 'startsplit', 'setup',
